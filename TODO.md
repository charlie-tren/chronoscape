...
#### Phase 1 - turn the spike into a real site
- [ ] Promote `spike/` to the repo root as `site/` (or keep the name; just stop calling it a spike).
- [ ] **Hash deep links**: `#event-42` in the URL on selection, read on load, `history.replaceState` so it does not spam back-history. Makes every event shareable without extra pages.
- [ ] **Open Graph + canonical tags** per country page (currently: none). Needed for the link to look right when shared and for the hub to link cleanly.
- [ ] **`sitemap.xml` + `robots.txt`**, generated by the build. Without these the SEO argument does not actually land.
- [ ] **404 page.**
- [ ] **Move the JSON Schema validation into the repo** and run it in `build.py`, failing the build on bad data. The Ireland build had this but it lived in a scratchpad script. This is what stops a malformed country reaching production.
- [ ] **A11y finish**: visually-hidden `<ul>` of events as a text alternative (research recommendation, not yet done); verify the roving-tabindex keyboard path end to end.
- [ ] Decide favicon (currently an inline emoji data URI - fine, but a real icon is nicer).
...
- [ ] Add `requirements-build.txt` (just `jinja2`) and `.python-version`.
...
#### Phase 5 - accounts (only if actually wanted)
- [ ] Clerk + Neon, following the security rules recorded further down. A static site plus Clerk is fine; saved views and share links need the Neon tables. Do not start this until phases 1-3 are done and soaked.
